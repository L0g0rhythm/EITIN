...
function Format-EitinDataToTxtSection {
    ...
    process {
        ...
        function Format-ObjectProperties {
            ...
            if ($null -ne $props['Error'] -and -not [string]::IsNullOrWhiteSpace($ItemObject.Error)) {
                $errorLabel = "Erro"; $errorValue = $ItemObject.Error
                if ($null -ne $props['Detalhe'] -and -not [string]::IsNullOrWhiteSpace($ItemObject.Detalhe)) { $errorValue += " (Detalhe: $($ItemObject.Detalhe))" }
            ...
            } elseif ($null -ne $props['Informação'] -and -not [string]::IsNullOrWhiteSpace($ItemObject.Informação)) {
